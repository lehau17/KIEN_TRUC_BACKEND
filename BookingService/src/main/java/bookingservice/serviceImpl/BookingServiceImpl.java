...
public class BookingServiceImpl implements BookingService {
    ...
    public boolean confirmBooking(String id) {
        ...
        if (result) redisTemplate.delete("bookings:all");
        return result;
    }
    ...
    private boolean updateAndSendMessage(String id, BookingStatus requiredStatus, BookingStatus newStatus,
        ...
        if (bookingOpt.isEmpty()) throw new RuntimeException("Booking không tồn tại");

        Booking booking = bookingOpt.get();
        if (booking.getStatus() != requiredStatus) return false;
    ...
    public List<BookingResponse> getBookingsByDate(LocalDate date, String type) {
        ...
        List<BookingResponse> cachedBookings = (List<BookingResponse>) redisTemplate.opsForValue().get(cacheKey);
        if (cachedBookings != null) return cachedBookings;

        List<Booking> bookings = ("checkin".equalsIgnoreCase(type)) ?
                bookingRepository.findByCheckInAt(date) :
